...
public class ListFactory {
	...
	public static Equipment choiceWeapon(Scanner sc, int type, String name,
			Map<String, Map<Integer, List<Equipment>>> equipment) {
		Utils.p("Choose " + name);
	...
	public static boolean battle(List<Enemy> enemies, List<Player> players, Scanner sc, Random rand, int battleNum) {
		...
		case 1:
			...
			enemies.add(new Skeleton("skeleton0",100));
			enemies.add(new Skeleton("bob",100));
			break;
		case 2:
	...
	public static boolean enemyAttack(List<Enemy> enemies, List<Player> players, Scanner sc, Random rand) {
		...
		for (Enemy enemy : enemies) {
			...
			Utils.p(enemy.getName() + " has attacked " + target.getName() + "!  " + target.getName() + " now has "
